...
function openTranscriptSlot(_ref) {
  ...
  return {
     ...
     */
    text() {
      return tokens.filter(token => token.isFinal).map(token => token.text).join(" ").trim();
    }
  };
}
...
export { DEFAULT_MAX_PROVIDER_MS, MAX_PROVIDER_RESTARTS, SETTLE_GRACE_MS, SUPPORTED_TIERS, TRANSCRIPT_AUTHORITY, clearTranscriptProviders, createBrowserSpeechProvider, openTranscriptSlot, registerTranscriptProvider };
