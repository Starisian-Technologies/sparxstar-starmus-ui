...
export function openTranscriptSlot({
    ...
    for (const factory of providerFactories) {
        ...
        let candidate;
        try {
            candidate = factory.create({ language, tier, sessionId });
        } catch (error) {
            console.warn(
                `[Transcript] Provider "${factory.name}" could not be created:`,
    ...
    const context = {
        emit(token) {
            ...
            if (provider.providesTimings === true && !measured) {
                console.warn(
                    "[Transcript] Provider declares timings but this token's were unusable; falling back to the recorder clock.",
                );
            }
            ...
            const entry = {
                ...
                startMs: measured
                    ? Math.round(token.startMs)
                    : supersedesInterim
                      ? tail.startMs
                      : Math.min(lastStartMs, endMs),
                endMs,
